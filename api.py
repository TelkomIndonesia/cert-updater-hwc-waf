...
def get_token(IAMDomain, IAMUser, IAMPassword, projectName, url):
    payload = {
        "auth": {
            "identity": {
                "methods": ["password"],
...
def upload_certificate(projectName, projectId, enterpriseProjectId, cert_path, key_path, cert_name, token):
    timestamp = int(time.time())
    cert_name = cert_name + "_" + str(timestamp)
    # Construct the URL.
    url_upload_cert = f"https://waf.{projectName}.myhuaweicloud.com/v1/{projectId}/waf/certificate?enterprise_project_id={enterpriseProjectId}"
    ...
    try:
        response = requests.post(url_upload_cert, headers=headers_auth, json=payload_upload_cert)
...
def apply_certificate(projectName, projectId, enterpriseProjectId, certificate_id, host_id, token):

    # Construct the URL.
    url_apply_cert = f"https://waf.{projectName}.myhuaweicloud.com/v1/{projectId}/waf/certificate/{certificate_id}/apply-to-hosts?enterprise_project_id={enterpriseProjectId}"
...
def get_prev_cert_id(project_name, project_id, host_id, enterpriseProjectId, token):
    
    # Construct the URL.
    url = f"https://waf.{project_name}.myhuaweicloud.com/v1/{project_id}/waf/instance/{host_id}?enterprise_project_id={enterpriseProjectId}"
...
def get_certificate_details(project_name, project_id, certificate_id, enterprise_project_id, token):
    url = f"https://waf.{project_name}.myhuaweicloud.com/v1/{project_id}/waf/certificate/{certificate_id}?enterprise_project_id={enterprise_project_id}"
...
def delete_certificate(project_name, project_id, certificate_id, enterprise_project_id, token):
    cert_details = get_certificate_details(project_name, project_id, certificate_id, enterprise_project_id, token)
    if not cert_details or cert_details.get("bind_host", []):
        print("Certificate is either not found or currently bound to hosts. Aborting deletion.")
        return False
    # Construct the URL.
    url = f"https://waf.{project_name}.myhuaweicloud.com/v1/{project_id}/waf/certificate/{certificate_id}?enterprise_project_id={enterprise_project_id}"
...
def get_host_details_by_cert_name(filename, cert_name):
    ...
    results = data.get(cert_name, {})
    
    
    return results


def main(IAMDomain, IAMUser, IAMPassword, projectName, url, cert_path, key_path, cert_name):
    # Step 1: Login and get token
    token = get_token(IAMDomain, IAMUser, IAMPassword, projectName, url)
    if not token:
    ...
    for item in cert_id:
        ...
        certificate_id = upload_certificate(projectName, project_id, enterprise_id, cert_path, key_path, cert_name, token)
        ...
        hosts = item['hosts']
        for host in hosts:
            prev_cert_id = get_prev_cert_id(projectName,project_id,host,enterprise_id,token)
            if not prev_cert_id:
                print("Failed to obtain Previous Certificate ID. Exiting!")
                return
            response_apply = apply_certificate(projectName, project_id, enterprise_id, certificate_id, host, token)
            if not response_apply:
                print("Failed to apply certificate. Exiting!")
                return
        print(prev_cert_id)
        delete_certificate(projectName, project_id, prev_cert_id, enterprise_id, token)

            # if not delete:
            #     print("Failed to delete previous certificate. Exiting!")
            #     return
